...
namespace ng
{
	...
	PUBLIC ranges_t sanitize (buffer_api_t const& buffer, ranges_t const& selection);
	PUBLIC bool not_empty (buffer_api_t const& buffer, ranges_t const& selection);
	PUBLIC bool multiline (buffer_api_t const& buffer, ranges_t const& selection);
	PUBLIC ranges_t move (buffer_api_t const& buffer, ranges_t const& selection, move_unit_type const unit, layout_movement_t const* layout = NULL);
	PUBLIC ranges_t extend (buffer_api_t const& buffer, ranges_t const& selection, select_unit_type const unit, layout_movement_t const* layout = NULL);
	PUBLIC ranges_t extend_if_empty (buffer_api_t const& buffer, ranges_t const& selection, select_unit_type const unit, layout_movement_t const* layout = NULL);
	PUBLIC ranges_t select_scope (buffer_api_t const& buffer, ranges_t const& selection, scope::selector_t const& scopeSelector);
	PUBLIC ranges_t toggle_columnar (ranges_t const& selection);
	PUBLIC ranges_t deselect_last (ranges_t const& selection);
	PUBLIC scope::context_t scope (buffer_api_t const& buffer, ranges_t const& selection, std::string const& extraAttributes = NULL_STR);
	PUBLIC ranges_t highlight_ranges_for_movement (buffer_api_t const& buffer, ranges_t const& oldSelection, ranges_t const& newSelection);
	PUBLIC std::map< range_t, std::map<std::string, std::string> > find (buffer_api_t const& buffer, ranges_t const& selection, std::string const& searchFor, find::options_t options, ranges_t const& searchRanges = ranges_t(), bool* didWrap = nullptr);
	PUBLIC std::map< range_t, std::map<std::string, std::string> > find_all (buffer_api_t const& buffer, std::string const& searchFor, find::options_t options, ranges_t const& searchRanges = ranges_t());
	PUBLIC range_t word_at (buffer_api_t const& buffer, range_t const& range);
	PUBLIC ranges_t all_words (buffer_api_t const& buffer);

	PUBLIC ranges_t dissect_columnar (buffer_api_t const& buffer, ranges_t const& selection);
	...
	PUBLIC std::string character_class (buffer_api_t const& buffer, size_t index);

	PUBLIC ranges_t from_string (buffer_api_t const& buffer, std::string const& str);
	PUBLIC std::string to_s (buffer_api_t const& buffer, ranges_t const& ranges);
