...
- (void)loadView
{
	...
	std::string const& promptForType = "";
	NSMenuItem* item = [unknownDocumentTypesMenu addItemWithTitle:[NSString stringWithCxxString:"Prompt for type"] action:@selector(selectUnknownFileType:) keyEquivalent:@""];
	[item setRepresentedObject:[NSString stringWithCxxString:promptForType]];
	[item setTarget:self];
