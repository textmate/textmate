...
namespace ng
{
	...
	CGFloat layout_t::height () const { return _margin.top + content_height() + _margin.bottom + (_scrollPastEnd ? std::min(_rows.aggregated()._height, _viewport_size.height) - row_for_offset(_buffer.lines()-1)->key._height * 2 : 0); }
