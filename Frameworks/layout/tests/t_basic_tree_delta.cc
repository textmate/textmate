...
class DeltaTreeTests : public CxxTest::TestSuite
{
	struct annotation_t
	{
...
public:
	void setUp ()    { random_insert(tree, buffer); }
	void tearDown () { TS_ASSERT(tree.structural_integrity()); tree.clear(); }

	void test_integrity ()
	{
		TS_ASSERT(tree.structural_integrity());
		TS_ASSERT_EQUALS(tree.aggregated().number_of_children, tree.size());
	}
	...
	void test_aggregation ()
	{
		...
		TS_ASSERT_EQUALS(maxLen, tree.aggregated().width);
	}
};
