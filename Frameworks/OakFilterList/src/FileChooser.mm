...
namespace
{
	...
	static filter_string_t parse_filter_string (std::string const& filter)
	{
		filter_string_t res;
	...
	{
		document_record_t (document::document_ptr const& doc, std::string const& projectPath)
		{
			identifier    = doc->identifier();
			full_path     = doc->path();
			relative_path = path::relative_to(full_path, projectPath);
...
- (id)init
{
	if((self = [super init]))
	{
		...
		_window.frameAutosaveName           = @"FileChooser";
...
- (void)addRecordsForDocuments:(std::vector<document::document_ptr> const&)documents
{
	...
	NSUInteger const firstDirty = _records.size();
	NSUInteger index = _records.size();
	for(auto doc : documents)
	{
		if(uuids.find(doc->identifier()) == uuids.end())
		{
...
inline void rank_record (document_record_t& record, filter_string_t const& filter, path::glob_list_t const& glob, std::vector<std::string> const& bindings)
{
	...
	if(filter.suffix != NULL_STR && record.name.rfind(filter.suffix) != record.name.size() - filter.suffix.size())
		return;

	if(double rank = oak::rank(filter.filter, record.name, &record.cover))
	{
		record.matched = true;

		size_t bindingIndex = std::find(bindings.begin(), bindings.end(), record.full_path) - bindings.begin();
		if(filter.raw_filter == record.name)
			record.rank = -0.1;
...
{
	filter_string_t filter = parse_filter_string(to_s(_filterString));
	path::glob_list_t glob("*");

	std::vector<std::string> bindings;
	for(NSString* str in [[OakAbbreviations abbreviationsForName:@"OakFileChooserBindings"] stringsForAbbreviation:[NSString stringWithCxxString:filter.filter]])
...
- (void)setPath:(NSString*)aString
{
	...
	_scanner.reset(new document::scanner_t(to_s(_path), globs_for_path(to_s(_path))));
...
- (void)updateStatusText:(id)sender
{
	...
	else
	{
		...
		std::string prefix = path::with_tilde(path::parent(record.full_path));
		std::string path = (prefix != NULL_STR ? prefix + "/" : "") + record.name;
		size_t offset = prefix.size() + 1;
...
- (NSArray*)selectedItems
{
	...
	for(NSUInteger i = [indexes firstIndex]; i != NSNotFound; i = [indexes indexGreaterThanIndex:i])
	{
		...
		auto filterString = parse_filter_string(to_s(_filterString));
		if(filterString.selection != NULL_STR)
			item[@"selectionString"] = [NSString stringWithCxxString:filterString.selection];
...
- (void)accept:(id)sender
{
	...
	if(_filterString)
	{
		std::string filter = parse_filter_string(to_s(_filterString)).filter;
		...
		for(NSUInteger i = [indexes firstIndex]; i != NSNotFound; i = [indexes indexGreaterThanIndex:i])
		{
			...
			if(record.full_path != NULL_STR)
				[[OakAbbreviations abbreviationsForName:@"OakFileChooserBindings"] learnAbbreviation:[NSString stringWithCxxString:filter] forString:[NSString stringWithCxxString:record.full_path]];
		}
	}
