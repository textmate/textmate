...
{
	NSPoint p = [self convertPoint:[[self window] convertRectFromScreen:(NSRect){ thePoint, NSMakeSize(0, 0) }].origin fromView:nil];
	std::string const text = editor->as_string();
...
- (id)accessibilityAttributeValue:(NSString*)attribute forParameter:(id)parameter
{
	...
	if(false) {
		...
		point = [[self window] convertRectFromScreen:(NSRect){ point, NSMakeSize(0, 0) }].origin;
...
- (NSPoint)positionForWindowUnderCaret
{
	...
	if(NSPointInRect(p, [self visibleRect]))
			{ p = [[self window] convertRectToScreen:[self convertRect:(NSRect){ p, NSMakeSize(0, 0) } toView:nil]].origin; }
...
- (void)showMenu:(NSMenu*)aMenu
{
	...
	NSEvent* fakeEvent = [NSEvent
		...
		location:[win convertRectFromScreen:(NSRect){ [self positionForWindowUnderCaret], NSMakeSize(0, 0) }].origin
		modifierFlags:0
