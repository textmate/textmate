...
- (id)initWithOutlineView:(NSOutlineView*)anOutlineView
{
	if((self = [super init]))
	{
		...
		_countOfLeafsButton = countOfLeafs;
...
- (void)outlineViewItemDidExpandCollapse:(NSNotification*)aNotification
{
	...
	if(item == self.objectValue)
		self.countOfLeafs = [outlineView isItemExpanded:item] ? nil : [NSString stringWithFormat:@"%lu", item.countOfLeafs];
}
...
- (NSView*)outlineView:(NSOutlineView*)outlineView viewForTableColumn:(NSTableColumn*)tableColumn item:(FFResultNode*)item
{
	...
	else
	{
		...
		cellView.countOfLeafs = [outlineView isItemExpanded:item] ? nil : [NSString stringWithFormat:@"%lu", item.countOfLeafs];
	}
	return res;
}
